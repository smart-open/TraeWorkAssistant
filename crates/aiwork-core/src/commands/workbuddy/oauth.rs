...
//!   ② Web 前端展示 authUrl 链接（Web 化改造：不再由后端打开系统浏览器；
//!       缺失或缺 state 凭证参数时按已验证形态 `copilot.tencent.com/login?platform=CLI&state=<state>` 构造，
//!       绝不展示裸链接）
...
//!
//! Web 化改造：桌面事件推送改回调（WbOauthEmitter → server 桥到 SSE），
//! 删除 open_in_browser（cmd /c start + CommandExt）调用。
...
/// 事件名 = "wb-oauth-progress"（进度/登录链接）| "wb-oauth-done"（终态），
/// 与桌面 emit 事件同名。
pub type WbOauthEmitter = std::sync::Arc<dyn Fn(&str, serde_json::Value) + Send + Sync>;
...
/// ① 候选字段链取第一个 http(s):// 开头且 query 已含 `state` 凭证参数的 authUrl **原样展示**
...
///    绝不原样展示缺 state 的登录页链接，也绝不回退裸基础域 URL。
...
/// OAuth 主流程（后台线程执行）：进度经 wb-oauth-progress 回调推送，结果经 wb-oauth-done。
fn oauth_flow(state: &AppState, emit: &WbOauthEmitter) -> Result<(String, String), String> {
    ...
    let emit_progress = |stage: &str, message: &str, auth_url: Option<&str>| {
        let _ = emit(
            "wb-oauth-progress",
    ...
    emit_progress("init", "正在请求登录 state…", None);
    let mut req = agent
    ...
    // ② 登录页链接推送（Web 化：前端展示 authUrl 链接，不再打开系统浏览器）
    emit_progress(
        "browser",
        "请在打开的登录链接完成扫码/登录（完成后停留在结果页即可，无需复制内容）",
        Some(&auth_url),
    );
    // 观测（一条）：auth/state 响应顶层键名 + data 内层键名 + 最终登录 URL
    ...
    fs_utils::app_log(
        &state.data_dir,
        &format!(
            "workbuddy: OAuth auth/state 响应键[{}] data键[{}]，登录链接 {auth_url}",
            keys_of(&body),
    ...
    while std::time::Instant::now() < deadline {
        ...
        polls += 1;
        if polls % 10 == 1 {
            emit_progress("polling", "等待登录完成…（最长 300 秒）", None);
        }
        let mut req = agent
    ...
    emit_progress("success", &format!("账号「{label}」已扫码登录并自动入池"), None);
    Ok((target_id, label))
}

/// OAuth 扫码登录（F-50）：后台线程执行全流程，事件回调驱动 UI；同时仅允许一个流程。
/// authUrl 经 "wb-oauth-progress" 回调推送，由 Web 前端展示登录链接。
...
mod oauth_tests {
    ...
    /// ① 服务端 authUrl 已含 state → 原样展示，不补参不构造
    ...
    ///    state 缺失/为空 → 明确报错，绝不展示无 state 的链接
    #[test]
    fn resolve_auth_url_missing_or_invalid_errors() {
        ...
        // 非法 authUrl（javascript:）视同缺失 → 构造，绝不展示
        assert_eq!(
