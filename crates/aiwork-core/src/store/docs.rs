...
pub fn pay_status_load(s: &Store) -> crate::legacy_types::PayStatusFile {
    let statuses = s
        .rows_all("pay_status")
        .unwrap_or_default()
        .into_iter()
        .filter_map(|(uid, data)| {
            serde_json::from_value::<crate::legacy_types::PayStatusEntry>(data)
                .ok()
                .map(|e| (uid, e))
        })
        .collect();
    let updated_at = s.kv_get::<Option<String>>("pay_status_updated_at");
    crate::legacy_types::PayStatusFile { statuses, updated_at }
}

pub fn pay_status_save(s: &Store, f: &crate::legacy_types::PayStatusFile) -> Result<(), String> {
    let rows: Vec<(String, Value)> = f
        .statuses
        .iter()
        .map(|(uid, e)| serde_json::to_value(e).map(|v| (uid.clone(), v)))
...
pub fn doubao_pool_load(s: &Store) -> crate::legacy_types::DoubaoAccountPool {
    let accounts = s
        .rows_all("doubao_accounts")
        .unwrap_or_default()
        .into_iter()
        .filter_map(|(_, data)| serde_json::from_value::<crate::legacy_types::DoubaoAccount>(data).ok())
        .collect();
    let meta: Value = s.kv_get("doubao_pool_meta");
    crate::legacy_types::DoubaoAccountPool {
        accounts,
        last_keepalive_at: meta.get("last_keepalive_at").and_then(Value::as_str).map(String::from),
    }
}

pub fn doubao_pool_save(s: &Store, pool: &crate::legacy_types::DoubaoAccountPool) -> Result<(), String> {
