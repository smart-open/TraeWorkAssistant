...
//! Tauri 应用入口：窗口/托盘/插件注册、单实例防护与启动流程。
...
fn main() {
    ...
    let app = {
        ...
        // 还原/显示/聚焦后，第二进程由插件自动退出。必须最先注册（在创建窗口前持有互斥锁）。
        // dev 模式不启用：dev 与已安装版共用 identifier（com.traework.assistant），
        // 启用会导致 `npm run tauri dev` 与已安装应用互相顶替退出，干扰开发调试。
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::pay_status::refresh_pay_status,
            commands::trae_local::apps_entitlement_read,
            commands::trae_local::apps_accounts_discover,
            commands::trae_local::apps_account_add,
