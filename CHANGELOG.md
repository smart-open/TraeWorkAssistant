...
- **日志轮询失败 toast 风暴**：运行日志自动刷新（2s）期间读取持续失败（文件锁/权限异常）时，只在首次失败弹一次错误 toast，恢复成功后重置；查询加并发序号去重（最新请求胜出），慢响应不再覆盖新数据（store.ts）。
...
- **API 服务未配 Key 默认拒绝（S2 missing_authz）**：未配置任何启用 Key 时业务请求一律返回 401 `auth_not_configured`（原为匿名放行），防止本机任意进程无鉴权借用上游凭证消耗额度；仅 `/health` 豁免；API 服务页与文档文案同步更新。
