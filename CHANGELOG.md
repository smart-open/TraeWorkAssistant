...
- **API 服务鉴权开关按钮文案与状态相反**：按钮应显示与当前状态相反的目标操作——鉴权关闭时显示「开启鉴权」（emerald 正向色）、开启时显示「关闭鉴权」（amber 警示色）；原实现文案与颜色均随当前状态而非目标动作渲染，与实际点击效果相反（ApiService.tsx）。
- **签到全部跳过时结果提示一闪而过**：全部账号被跳过（今日已签 / JWT 过期 / 冷却中）时后端发 `total=0` 空轮次，实时进度卡片不渲染且完成 toast 4.2s 自动消失，用户不知道为何没有签到。现改为：`pushToast` 支持 sticky（不自动消失需手动关闭），全跳过场景发 sticky 警告并注明跳过原因；实时进度卡片在全跳过后也渲染完成态摘要与原因提示条（store.ts + Checkin.tsx）。

## [2.9.2] - 2026-09-10

### 修复

- **日志轮询失败 toast 风暴**：运行日志自动刷新（2s）期间读取持续失败（文件锁/权限异常）时，错误 toast 限频为每 60s 最多一次（手动查询直通即时反馈）；轮询在飞时跳过本轮防堆积，并发请求按序号「最新胜出」，慢响应不再覆盖新数据（Logs.tsx + store.ts）。

### 安全

- **更新包完整性校验（S1 insecure_update）**：发布时 `rename_release.py` 生成校验清单 `latest.json`（版本 + 各资产 SHA-256）并随 Release 上传；`update_check` 下载清单取出发布方摘要（fail-closed：清单缺失/损坏/版本不符即阻止自动更新，引导手动下载）；`update_download` 下载完成后与清单强制比对，不匹配删除并拒绝；安装前仍与下载记录二次比对（防替换）。
- **API 服务未配 Key 默认拒绝（S2 missing_authz）**：未配置任何启用 Key 时业务请求一律返回 401 JSON 引导文案（原为匿名放行），防止本机任意进程无鉴权借用上游凭证消耗额度；「API 服务」页新增显式「鉴权开关」（关闭鉴权不推荐并标注风险；存在启用 Key 时开关无效）；仅 `/health` 豁免；文档文案同步更新。
...
- **API 网关用量统计**：请求按日聚合落盘 `data/api_usage.json`（日期/协议端点/模型/账号/Key/成败/流式/耗时维度），保留 90 天；API 服务页新增「用量统计」面板（近 N 天请求量/成功率图表 + 明细）。
- **多 API Key 管理**：`data/api_keys.json` 支持多 Key（名称/启停/每日限额，0=不限），超配额返回 429；未配置任何启用 Key 时不鉴权。
- **托盘菜单增强**：托盘新增「立即签到」（跳过已签账号）与「启动/停止 API 服务」（菜单文本随状态切换），签到完成/服务状态变化弹系统通知；与页面操作防重入。
- **敏感数据加密（Stronghold）**：账号 jwt / refresh_token 迁移至 `conf/vault.stronghold`（主密码经 Windows DPAPI 加密），`checkin_accounts.json` 不再存明文凭据；Python 签到脚本改用临时解密文件，进程结束即删。
- **签到失败自动重试**：失败账号自动重试最多 2 轮（间隔 30s → 90s 逐轮加长），UI 展示重试倒计时横幅，最终统计以最后一轮为准。
- **签到成功率趋势**：每日 per-uid 签到结果落盘 `data/checkin_results.json`（重试轮合并为最终态，保留 90 天）；Dashboard 新增「近 30 天签到结果」堆叠柱状图。
- **OpenAI 兼容端点扩展**：新增 `POST /v1/completions`（legacy text completion，支持流式/非流式）；`/v1/embeddings` 返回 501 + 明确说明（上游无对应能力）。
- **账号池调度策略**：`api_pool.json` 支持 `strategy`（积分先过期优先 / 剩余积分多优先 / 随机）与 `group_ids` 分组筛选（空=全部），保存后重启 API 服务生效。
- **开机自启 + 启动静默签到**：设置页新增两个开关；开启静默签到后应用启动 60s 自动对未签到账号执行一轮签到（幂等，完成后系统通知）。
- **前端测试基建（Vitest）**：`npm run test` 落地，覆盖 `cn` / `withMinDelay` / `maskApiKey` / `fmtTokens` 等纯函数（13 例）；核心模块补中文模块级文档注释。

### 修复

- **vault 重启后凭据丢失**：快照重开后必须显式 `load_client`，原先空 client 覆盖快照会导致全部凭据丢失（代码审查 Critical）。
- **关闭鉴权后旧客户端 401**：未配置任何鉴权时，携带旧/未知 Key 的请求放行并记为 anonymous，对齐旧版「无 Key 即全放行」行为。
- **日限额输入校验**：空/非法输入不再静默存为 0（不限），还原原值并提示。

### 变更

- **移除主 API Key**：取消「主 Key + 子 Key」双轨校验，API Key 统一在「API Keys 管理」列表维护（启用 Key 为空时不鉴权）；设置页不再展示主 Key 输入框，旧 settings.json 中的 api_key 字段自动忽略。
- **日志页支持按类型清理**：新增「清理」按钮（确认弹窗 + 结果反馈），可清理单类或全部日志文件。
- **分组筛选实时预览**：点选分组即时显示将纳入的账号数，账号列表对分组外账号标记并置灰。
- **Keys 表单位标注**：日限额/今日已用标清「次」，并展示该 Key 当日 token 消耗。
- **临时凭据文件加固**：Python 签到临时文件改存应用数据目录（不再落全局 %TEMP%），启动时清理崩溃残留。
...
- **「关于」弹窗改版**：移除打赏/赞赏码图片及其逻辑，「软件宣传图」全新上线（琥珀金盾牌 + 多账号管理视觉，图内含软件名称与标语）；图片以 base64 内嵌，离线可显示
- 新增临时出图目录 `out/` 至 .gitignore
...
- **API 服务页 API Key 支持一键复制**：「查看」图标左侧新增复制图标，点击复制完整 API Key（非打码值）到剪贴板，图标短暂变绿 ✓ 并提示成功；Key 为空时隐藏复制入口；「发现新版本」横幅与自更新流程完整可用（自 2.8.2 起）。

---

## [2.8.2] - 2026-09-08

两轮代码审查批量修复：UI 冻结清零、错误分类与进程查杀边界修正、网关健壮性加固，升低位版本（2.8.1 → 2.8.2）。

### 修复

- **UI 冻结清零**：环境探测（`env_check`，注册表回退路径可达数秒）、打开 Trae（三级关闭最长约 5s 轮询）、计划任务注册/查询/注销（schtasks 约 1s）全部改为 `#[tauri::command(async)]` 派发，主线程不再阻塞，配合 2.8.1 的更新器 async 化实现全应用零冻结。
- **检查更新资产版本回填必败**：资产名版本低于 tag 版本时，回填改为「替换」下划线倒数第二段版本段（原先追加后缀导致 `version_from_asset` 解析错位，下载前置校验必失败）。
- **安装包误降级**：非 WiX 场景覆盖安装时，降级拦截从不可达的 leave 回调前移至页面 create 回调（`Quit` 终止安装），与静默模式 EarlyChecks 行为一致；自更新模式（/UPDATE）放行。
- **账号误冷却 12 小时**：错误分类中宽泛的 `contains("plan")` 收紧为 `plan limit`/`plan_limit`/`plan quota` 短语 + 精确 `"code":1005` JSON 键，"planned maintenance" 等消息不再误判为套餐耗尽。
- **流式对话永久挂起**：流式上游 Agent 补 300s 空闲读超时（仅约束单次 read 等待，正常 SSE 不受影响），上游建立连接后长期不发数据不再挂死任务线程与客户端连接。
- **流式中文错误 panic**：`safe_slice` 改用 `get(..n)` UTF-8 安全截断，上游错误 JSON 含中文时不再因字节边界 panic（表现为截断空流 / 500）。
- **工具流中途错误重复下发**：`sent_any` 计入就地透传错误，消除双重 error 事件与误触发重试重放。
- **OpenAI 端点非法请求**：`/v1/chat/completions` 增加 JSON 与 `messages` 必填校验，非法请求返回 400 而非上游 502。
- **进程查杀误报人工介入**：三级关闭超时后增加终判兜底再探测，超时竞态窗口内恰好退出的进程不再误报。
- **子进程代理行为污染**：移除 API 服务启动时的进程级 `set_var("NO_PROXY","*")`（多线程 setenv 竞态 + 污染 python 等子进程环境；ureq 未启用 proxy-from-env 本就不读环境代理）。

### 变更

- **`api_models.json` 归位 `data/` 子目录**：与其他数据文件布局一致，旧位置文件启动时幂等迁移，自学习覆盖不丢失。
- **`read_text_file` 收紧**：仅允许读取 `.json` 文件（唯一用途为账号导入），阻断前端被注入后读取任意本地文件。
- 「关于」对话框：更新包大小未知（回填置 0）时显示「未知大小」。
