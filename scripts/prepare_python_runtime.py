...
使 tauri.conf.json 现有资源映射 `../src-python/ -> python/` 无需改动即可把
解释器 + 依赖 + 脚本一并打进安装包；应用启动时 state.rs 检测到
python/python.exe 存在即优先使用内嵌解释器，不再回退系统 Python。
