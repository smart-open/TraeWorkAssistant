...
> **功能需求、业务价值、实现逻辑与关键代码参考**，并附本会话代码审查的修复记录。
> 所有改动已完成开发与自测（`cargo test` 33/33、`tsc` 通过、`vitest` 13/13、`vite build` 成功），
> 已随 **v2.9.0** 交付；发布前追加调整「移除主 API Key」见[第 15 节](#15-追加调整移除主-api-keyv290)。
...
**主要修改文件（Rust）**：[main.rs](../src-tauri/src/main.rs)（托盘/自启/静默签到）、[checkin.rs](../src-tauri/src/commands/checkin.rs)（重试/防重入/落库）、[auth.rs](../src-tauri/src/api_server/auth.rs)、[pool.rs](../src-tauri/src/api_server/pool.rs)、[routes.rs](../src-tauri/src/api_server/routes.rs)、[api_server.rs（commands）](../src-tauri/src/commands/api_server.rs)、[misc.rs（commands）](../src-tauri/src/commands/misc.rs)，以及 accounts/oauth/pay_status/trae_local 等统一切换 `vault::load_accounts / save_accounts`

**主要修改文件（前端）**：[ApiService.tsx](../src/pages/ApiService.tsx)（统计面板 + Key 管理 + 池策略）、[Checkin.tsx](../src/pages/Checkin.tsx)（重试横幅）、[Dashboard.tsx](../src/pages/Dashboard.tsx)（趋势图）、[Logs.tsx](../src/pages/Logs.tsx)（清理按钮）、[Settings.tsx](../src/pages/Settings.tsx)（自启开关）、[store.ts](../src/store.ts)、[tauri.ts](../src/lib/tauri.ts)
...
前端开关（[Settings.tsx](../src/pages/Settings.tsx#L52-L65)、[L270-L275](../src/pages/Settings.tsx#L270-L275)）：
...
| 验证项 | 结果 |
|--------|------|
| `cargo test`（Rust 单测：vault 快照往返/DPAPI、api_keys 配额、pool 策略与分组、usage 聚合裁剪、checkin_results 合并、models_sync 迁移等） | **33/33 通过**（含移除主 Key 迁移用例后） |
| `npx tsc --noEmit` | 通过 |
| `npm run test`（Vitest） | **13/13 通过** |
| `npx vite build` | 成功（仅存量 chunk 体积告警） |

**交付状态**：版本号已定 **2.9.0**（`npm run set-version 2.9.0` 同步 Cargo.toml / package.json / 文档），主 Key 移除调整已合入并通过全部验证，NSIS / portable / MSI 编译打包完成，提交 → 推送 → tag `v2.9.0` → GitHub Release（3 资产）发布流程随后执行。
...
- `cargo test` 33/33（原 34 个，移除 1 个主 Key 迁移用例）、`npx tsc`、`npm run test` 13/13、`vite build` 全部通过
