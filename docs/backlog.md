...
> **⚠️ Web 化转型说明（2026-09-21）**：桌面壳已整体退役（登录态切换/豆包/代理/计划任务等桌面专属能力随之消失），本清单中依赖桌面模块的条目（如切换器/代理/豆包相关 F-xx）在 Web 版语境下不再适用或已随模块退役，新需求一律以 Web-only 架构（crates/aiwork-core + aiwork-server）为前提评估。
