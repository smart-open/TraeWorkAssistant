...
| 编号 | 需求 | 验收标准 |
|---|---|---|
| F-09 | token 续期：首选 `POST /v2/plugin/auth/token/refresh`（`X-Refresh-Token` 头）；备选 Keycloak 原生端点 | 惰性刷新（临期 <24h 才刷）；每账号互斥锁；失败标 `needs_relogin` + 原因 + 桌面通知；schtasks 每周兜底 |
...
| F-22 | 多账号余额聚合 + 趋势图（P1）：Credits 页 WorkBuddy Tab | 余额大字 + 月度消耗趋势 + 各账号对比条形图 |
...
| F-29 | 账号池调度引擎（P1）：三因子加权随机选号（credits×10 + 闲置补偿 + 成功率×3 → Top5 二次加权）+ 熔断 | hard_credit 冷却到次日 04:00 自动恢复；soft_rate 60s；100ms 窗口去重防惊群 |
| F-30 | 请求规范与改写层（P1）：Origin/Referer 必带、`X-No-*` 占位、强制 stream、`tool_choice` string 化、effort 降级、指纹清洗（可开关） | **红线：chat 请求绝不携带 X-Refresh-Token**；违反任一规范的上游 400 类错误在联调清单中逐项验证 |
| F-31 | 会话粘性路由（P2）：conversationId → 账号绑定，双段分配（先空闲账号哈希再全池哈希） | TTL 30m 滚动续期；写锁 re-check 防 TOCTOU |
| F-32 | 网关运维接口（P1）：`/v1/models`、`/status`（每账号画像）、`/healthz`（无健康账号 503） | 请求级日志（seq/TTFB/uid/tokens/latency） |
| F-33 | 错误三态分类（P1）：`RETRY_SAME` / `SWITCH_KEY` / `FATAL` | 硬编码标记词表（积分不足/insufficient credit/quota exceeded…）集中维护 |
...
- 新增 `workbuddy_env_check` 命令：客户端安装/运行状态、auth 文件存在性、`~/.workbuddy/storage/skeleton/account-snapshot.json` 可读性（当前登录 uid/昵称/editionType）。Dashboard 概览页 WorkBuddy 卡片展示。
...
- **调度**：`refresh_jwt` 同款惰性触发 + `schtasks` 每周兜底任务 `AIWorkAssistant_WorkBuddyRenew`；「一次查询最多一次刷新」——多路请求任一 401 统一刷新一次仅重试失败分支，**禁止二次刷新**（防旧 refresh token 覆盖新 token）。
...
| 展示 | `GET .../energy`（能量）、`GET .../streak`（连签天数） | Credits Tab 附注 |

**Tauri 侧**：`workbuddy_checkin_start(opts)` 命令 + Checkin 页 WorkBuddy 分区 + `workbuddy_checkin_task_register(time)` 每日任务（09:00/21:00 双时段，F-16）+ 失败桌面通知。
...
**落地形态**：`workbuddy_credits.py`（查询+缓存回写 `data/workbuddy_credits_cache.json`，≥5min 缓存）+ `workbuddy_credits_fetch` 命令；Credits 页 WorkBuddy Tab（余额大字 + 月度趋势 + 各账号对比条形图，F-22）；F-25 官方用量 / F-26 本地 token 统计（JSONL 解析按模型/项目/会话聚合）/ F-27 快照回退随批次 2~3 补齐。

### 3.7 UI 设计（v1.1 扩写：对齐 workbuddy-switch v0.3.1 实测界面）

> 参考界面：账号管理 / 全部积分包 / Token 统计 / 积分统计 / 设置 五张实测截图。信息架构差异：workbuddy-switch 是 WorkBuddy 专用单应用（左侧四页：账号管理/Token 统计/积分统计/设置）；本项目是四应用工作台，**不照搬其导航**，而是把各页设计吸收进既有页面结构（Accounts/Checkin/Credits/Settings + 新增 Token 统计视图）。底部状态条（「WorkBuddy：运行中 + 版本 + 有新版本」）吸收进 Dashboard WorkBuddy 卡片。

**页面映射总表**（本项目页面 → WorkBuddy 元素）：

| 页面 | WorkBuddy 元素 | 详细规格 |
|---|---|---|
| Dashboard | WorkBuddy 卡片：安装状态 / 当前账号 / 今日签到 / 池内账号数 / 总余额 / 客户端运行状态与版本（吸收 workbuddy-switch 底部状态条） | §3.2 |
| Accounts | 应用筛选器（Trae Work/Trae/豆包/WorkBuddy）+ 账号卡片区 + 聚合迁移入口 | §3.7.1 |
| Checkin | WorkBuddy 分区：签到范围 + 成长中心三开关（旅行/盲盒/任务）+ 双时段定时 + 结果逐账号 | §3.5 |
| Credits | WorkBuddy Tab = 积分统计（§3.7.4）+ Token 统计视图（§3.7.3）+ 到期提醒条 + 能量/连签附注 | §3.7.3/3.7.4 |
| ApiService | 上游类型展示（Trae llm_utils_chat / WorkBuddy copilot）+ 调度策略 + `/status` 账号画像入口 | §3.9 |
| Settings | WorkBuddy 配置区（环境/自动签到/签到日志/CLI 轮换/通知渠道） | §3.7.5 |
| 到期日历 | 统一组件：token 到期（access/refresh 双轨）、积分包到期（DeductionEndTime）、会员到期（复用 Trae 侧） | §3.7.2 |

#### 3.7.1 账号管理（Accounts 页 WorkBuddy 分区，F-54/F-60）

**顶部聚合区「添加与迁移账号」**（一张横向卡，副文案「快速导入账号，或从已有环境恢复」）：

| 入口 | 动作 |
|---|---|
| OAuth 扫码添加 | 打开扫码弹框（F-50：state → 浏览器扫码 → 轮询 → 自动入池） |
...
│ ──────────────────────────────────│
│ 活跃明细                            │
│ ▓▓▓▓░ 42.37 积分  运营裂变包  09/29 到期│  ← 按到期升序取前 2 个包：剩余+包名+到期
...
- 交互约束：「设为当前」= 调 `workbuddy_switch`（客户端关闭窗口期执行，NDJSON 进度）；「设为备用」仅改标记位（提示需点「设为当前」才真正切换登录态）。
- 卡片右上角徽标体系：editionType（free/pro）、token 临期（<24h 橙标 / 过期红标）、needs_relogin 红标、今日已签 ✓。

#### 3.7.2 积分包明细弹窗（F-56）

- 弹框头：「<昵称> · 共 N 个积分包」+ 关闭按钮。
- 逐包行：包名（如「CodeBuddy个人版国内运营裂变包」）+ 右侧 `剩余 / 总量`（如 `42.37 / 100`）与 `已用 X`；第二行到期日（`到期 2026/09/29`）+ 绿色进度条（占比 = 剩余/总量）；按到期日升序——**最先到期的排最前**（与 CLI 轮换「最早到期先用」口径一致）。
- 7 天内到期包：到期日红字 + 卡片「即将到期」角标（联动 F-13 到期日历）。

#### 3.7.3 Token 统计（F-57，Credits 页内新视图或独立子页）
...
- **Token 活动热力图**：GitHub 风格年度日历（列=周、行=星期，最近一年，灰→绿四档强度），右上「每日」粒度切换。

#### 3.7.4 积分统计（F-58，Credits 页 WorkBuddy Tab 上半区）

- **四 KPI 卡**：剩余积分 / 今日消耗 / 近 7 天消耗 / 本月消耗（大字 + 图标）；数据更新时间标注。
- **官方积分消耗堆叠柱**：副标题注明「来自 WorkBuddy 官方请求用量 · <起始> 至 <截止>」（官方接口有明确的统计窗口，UI 必须带口径说明）；「所有账号」筛选 + 时间范围；按模型分色堆叠（glm-5.2 / deepseek-v4-pro / minimax-m3 / deepseek-v4-flash / glm-5.3-flash / 其他）；脚注「当前窗口合计 X 积分」。
- **按模型分类排行**：模型数徽标（如「11 个模型」）+「共 N 次请求」；逐行：模型名 + 进度条 + 右侧 `请求数 / 合计积分 / 占比%`；按合计积分降序；**展示消耗最高的 8 个，脚注「已显示消耗最高的 8 个模型，其余模型仍计入上方合计」**（防长尾撑爆列表）。

#### 3.7.5 设置（Settings 页 WorkBuddy 配置区，F-55/F-59）

| 分区 | 字段 | 说明 |
|---|---|---|
| 环境 | 客户端路径 + 自动检测 | 复用 `app_locate`；显示探测来源与版本（workbuddy-switch 的「权限检测」为 macOS 专属，Windows 版替换为 auth 文件路径存在性校验 + 打开所在目录按钮） |
...
| | 操作 | 保存配置 / 全部立即签到 |
| 签到日志 | 最近 30 天滚动 | 逐条：账号名 + 结果（成功绿/失败红+原因如 `context deadline exceeded`）+ 时间；数据源=`workbuddy_checkin_results.json`（90 天存储，UI 默认展示 30 天） |
...
- 请求体改写：强制 `stream:true`；`tool_choice` 对象 → string（对象形式上游报 400 `code=11101`）；`reasoning_effort` 按模型 `supportedEfforts` 自动降级（档位序 off<minimal<low<medium<high<xhigh<max，snake/camel 双字段兼容）；指纹清洗可开关（剥离 Claude Code 痕迹：身份句改写、`cc_xxx=...;` 键值对剥离、`X-Anthropic-*` 头引用剥离，零分配预检 + 正则兜底）。
...
- 会话粘性（F-31）：conversationId/metadata 键 → 账号绑定；双段分配（先空闲账号哈希再全池哈希）；TTL 30m 滚动续期；写锁 re-check 防 TOCTOU。

**③ 错误三态（F-33）**：`RETRY_SAME`（502/503/超时 → 同 Key 重试 1 次）/ `SWITCH_KEY`（401/403/429 → 刷新换号）/ `FATAL`（400 context_too_long → 终止）；积分耗尽标记词表（积分不足/额度不足/insufficient credit/quota exceeded…）集中维护。
...
### 批次 1 · 快赢闭环（≈11~13 天）——目标：识别→切换→续期→签到→余额全通

| # | 任务 | 涉及 | 预估 | DoD 摘要 |
|---|---|---|---|---|
| T1.1 | `workbuddy_env_check` + Dashboard 卡片 | commands/workbuddy.rs（新建）、Dashboard.tsx | 0.5d | 四级探测返回 exe/authDir/dataDir；卡片展示当前账号与池统计 |
...
| T1.5 | workbuddy_checkin.py + Checkin 页分区（F-15） | src-python、Checkin.tsx | 2d | 已签容错；401 刷新重试；NDJSON；零 token 输出 |
| T1.6 | 积分三件套 + 旧接口回退 + Credits Tab（F-20/F-22） | workbuddy_credits.py、Credits.tsx | 2d | 双域路由；6 种嵌套解析；5min 缓存；到期提醒 |
| T1.7 | 到期日历（F-13，跨应用） | 新组件 + 三应用接入 | 1d | token/积分/会员三类到期统一日历 + 提醒 |
| T1.9 | 账号卡片区 UI：双态卡片 + 积分包明细弹窗 + 聚合迁移入口（F-54/F-56/F-60） | Accounts.tsx、新 AccountCard 组件 | 2d | 卡片布局/在线徽标/余额大字/活跃明细对齐 §3.7.1；明细弹窗逐包进度条（§3.7.2） |
| T1.10 | 自动签到配置化 + 启动补签（F-55） | Settings.tsx、checkin 管线 | 1d | keepalive_days / lazy_refresh_hours 参数化；启动核验未签自动补签；签到日志 30 天滚动 |
| T1.8 | 批次 1 集成测试 + 文档同步（AGENT.md/CHANGELOG） | docs | 0.5d | 全链路手工回归清单通过 |

### 批次 2 · API 暴露 + 成长中心（≈10~12 天）

| # | 任务 | 涉及 | 预估 |
|---|---|---|---|
| T2.1 | WorkBuddy 上游适配：headers 三铁律 + 强制 stream + tool_choice/effort 改写 + 非流式聚合（F-28/F-30） | api_server/payload.rs、新 upstream | 3d |
| T2.2 | 调度引擎：三因子加权 + 熔断 + 04:00 恢复（F-29）+ 错误三态（F-33） | api_server/pool.rs | 2.5d |
| T2.3 | 运维接口 /v1/models /status /healthz + 请求级日志（F-32） | api_server/routes.rs | 1d |
| T2.4 | 会话粘性双段分配 + TTL（F-31） | api_server | 1d |
...
|---|---|---|
| `src/pages/AccountsPage.tsx` | 754 | Accounts 页 WorkBuddy 分区（卡片区+聚合入口） |
| `src/pages/TokenStatsPage.tsx` | 1355 | Token 统计视图（总览卡/双轴趋势/热力图） |
| `src/pages/CreditStatsPage.tsx` | 1388 | Credits WorkBuddy Tab（KPI/官方消耗/模型排行） |
| `src/pages/SettingsPage.tsx` | 956 | Settings WorkBuddy 配置区 |
