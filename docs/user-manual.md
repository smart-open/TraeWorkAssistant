...
| 接口配置 | 监听端口、默认模型、使用示例、模型档位与 Max Mode 说明（端口改动下次启动服务后生效） |
...
- **账号池选择**：勾选参与调度的账号，配置调度策略（积分先过期优先 / 剩余积分多优先 / 随机）与分组筛选；保存后需重启服务生效。调度时自动跳过禁用 / 冷却中 / 积分已过期的账号，单账号额度耗尽自动换号（单请求最多 3 次）
...
> ⚠️ 该修复位于 Rust 代理模块中，请确认已升级到 v2.4.3 正式安装包（旧安装包内仍是旧实现）。
...
- `conf/` — 配置文件与凭证保险库
- `data/aiwork.sqlite` — 账号、积分、签到、网关等全部状态数据（单文件数据库）
- `data/profiles*/` — 各应用登录态快照
...
> 快照存储于 `data/profiles_doubao/<uid>/`，账号元数据存于本地状态数据库（`data/aiwork.sqlite`）。
