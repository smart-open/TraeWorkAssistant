...
- **其他操作**：恢复快照（不备份当前）、删除快照、编辑别名/备注/会话凭证、移出账号池。
...
- 额度接口（订阅/生图/视频）无公开文档，需先经抓包工具登录豆包会员中心捕获已登录 XHR 地址，在「环境配置 → 会员额度」填入并保存。
- 之后在「账号管理」对已录入会话凭证（sessionid，见 12.3）的账号点击额度按钮，即可查看会员等级、到期时间与剩余额度条。
