...
> 适用版本：v2.0.0 ｜ 平台：**仅 Windows 10 / 11**
> 本文档说明如何准备环境、启动开发、打包发布、日常运行与排错。
> v2.0.0 新增：本地 API 网关、代理日志、签到错误冷却状态机、积分过期感知调度、6 层设备标识重置。
...
- 菜单项从「运行日志」更名为「系统日志」，包含两个页签：
  ...
  - **代理日志**（v2.0 新增）：结构化代理请求列表，支持：
    ...
    - 时间段过滤（起止时间选择）
    - 时间倒序排列
    - 多日志文件分割查询（按日期自动分割 `proxy_logs/YYYY-MM-DD.log`）
...
- 启动后可通过 OpenAI 兼容协议调用：
  ...
  GET  http://127.0.0.1:7864/healthz
  ```
