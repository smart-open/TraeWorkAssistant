...
> 适用版本：v2.9.0 ｜ 平台：**仅 Windows 10 / 11**
...
- **多 API Key（T2/T15）**：`data/api_keys.json` 多 Key 独立签发 + 每日配额（0=不限，跨天重置，超限 429）；每请求重读文件，增删/启停立即生效；主 Key 双轨已移除，鉴权统一走 Key 列表（未配置启用 Key 时不鉴权，携带未知 Key 放行记 anonymous）。
