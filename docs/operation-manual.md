...
- 侧栏「API 服务」页提供多卡片布局：接口配置、账号池选择、运行状态、API Keys 管理、用量统计。
- **接口配置**：设置监听端口（默认 7864）与默认模型（如 `glm-5.2`）；API Key 统一在「API Keys 管理」卡片创建（未配置启用 Key 时不鉴权）。
- **账号池选择**：勾选要纳入 API 服务的账号，并可选调度策略（积分先过期优先 / 剩余积分多优先 / 随机）与分组筛选（不选分组 = 全部参与），保存后需重启 API 服务生效。
- **API Keys 管理**：多个 Key 独立签发并设置每日配额（0=不限），超限请求返回 429；增删/启停立即生效。
- **用量统计**：按日落盘的请求量 / 成功率 / Token 消耗 / 平均耗时，含近 N 天趋势图与模型分布（服务未运行也可查看）。
- **运行状态**：一键启停 API 服务，展示当前活跃账号、总请求数、账号池健康度。
- 启动后可通过 OpenAI 兼容协议调用：
  ```
  POST http://127.0.0.1:7864/v1/chat/completions
  POST http://127.0.0.1:7864/v1/completions
  POST http://127.0.0.1:7864/v1/messages（Anthropic 兼容）
  GET  http://127.0.0.1:7864/v1/models
  GET  http://127.0.0.1:7864/status
  GET  http://127.0.0.1:7864/health
  （/v1/embeddings 上游无对应能力，返回 501）
  ```
