...
注意：本文件位于 tests/ 目录，pytest 收集时会 import 本模块。所有网络请求、
文件读取与环境变量修改都必须收在 main() 内，保证「模块导入零副作用」，
否则会破坏 pytest 收集（历史问题：模块级发起网络请求导致收集阶段挂起/报错）。
仅限直接运行：`python src-python/tests/test_all_credits.py`
"""
